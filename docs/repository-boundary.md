...
独立 Client 发行版为 0.4.13（事实源为根目录 `VERSION`），Rust 契约版本仍为 0.4.0，移动状态 epoch 为 `media-backup-mobile-v0.4-r1`，移动 C/JNI 调用仍使用明确的 ABI v2。
...
仓库拆分保留原有发布约束；未触发签名或创建新 Release，也未将 Secrets 移到 Server。
