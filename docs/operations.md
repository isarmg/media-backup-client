...
本文只描述 `0.4.15` Client：Rust 移动核心与 FFI、Android 应用和 iOS 应用。Server、管理 Web、
...
`media-backup-ios-0.4.15-unsigned.ipa` 未签名；安装前必须用自己的 Apple 身份签名，打包本身不会授予
设备安装权限。
...
标签必须精确为 `v0.4.15` 并指向待发布提交。Release workflow 分别构建 Android 正式 APK、iOS 未签名
