...
本文以 Media Backup Client `0.4.15` 当前工作树为主，盘点 Rust Client、Android、iOS，并保留与独立
...
| MED-P-004 | Client 发行号为 `0.4.15`；持久合同只接受应用版本 `0.4.0`、revision 1 与 `media-backup-mobile-v0.4-r1` | `VERSION`、Client constants、release identity | 保障 | 高 | 把发行号误作状态版本会意外破坏现有队列 | 非当前 product/version/revision/epoch 零写入拒绝 |
...
| MED-P-009 | Client 固定 Foundation Client `0.9.16` 与完整 Git revision；protocol 也固定 Server 仓库完整 revision，无相邻工作区来源 | `sarmg-client.toml`、crate manifests、Cargo.lock | 保障 | 高 | 平台或 wire 行为可能随未固定依赖漂移 | 独立 checkout 的 locked 构建、合同与 ABI 测试 |
...
| ID | 当前功能/特性与真实行为 | 实现/代码锚点 | 分类 | 复杂度 | 删除后的确定后果 | 最低验证/边界 |
|---|---|---|---|---|---|---|
| MED-U-001 | 唯一移动 HTTP base 为 `/v2`，上传 DTO 使用 `deny_unknown_fields` | Server `crates/protocol`、`routes.rs`；Client 固定 Git 依赖 | 核心 | 高 | 三端 wire 漂移或歧义 JSON 被接受 | 全 DTO 正反 fixture、合同外 route |
...
| MED-G-008 | `next_prepared` 会复用 `ready` 及带准备结果的到期 `retry_wait`，再在进程内 Mutex 下领取一个未准备任务；当前没有跨进程 Client 锁 | `Client::next_prepared` | 保障 | 高 | 删除复用会重复读取并分块；误称跨进程安全会掩盖重复准备风险 | ready/retry 重取、双线程调用、两个进程边界、空队列、retry time |
...
| MED-G-012 | retryable error 使用 2 秒指数退避、最多 1 小时并保留、复用 `prepared_json`；非 retryable 进入 failed | `mark_failed`、`next_prepared` | 保障 | 中 | 网络故障会热循环，或永久失败与暂时失败无法区分 | retry count 0/10+、到期前后、JSON/part 是否仍在、错误摘要 |
...
| ID | 当前功能/特性与真实行为 | 实现/代码锚点 | 分类 | 复杂度 | 删除后的确定后果 | 最低验证/边界 |
|---|---|---|---|---|---|---|
| MED-R-001 | Server 软件为 0.3.21，数据库 Schema identity 为 media-backup 0.3.0、revision 5、SHA `a07c5723568cfcbf379a2173225122dc5db4e2168a50700d7f256aba3de5957e`；管理员和平台 DDL 由 Foundation 组合 | Server `database.rs`、`schema/generated/current_schema.sql` | 保障 | 高 | 错库或 DDL drift 必须拒绝 | metadata、现场 fingerprint、当前身份精确校验 |
...
| MED-R-015 | Server 安装 no-clobber 固定 `/opt/isarmg/media-backup/releases/0.3.21`，环境 0600 | Server `setup-wsl.sh`、deployment tests | 保障 | 高 | 同版本覆盖会让运行内容不可追溯，Secret 权限过宽 | 首装/二次安装、concurrent、mode/owner |
...
| MED-X-014 | 每次准备使用 job 下的新 UUID generation；新结果持久化后只回收该 job 的旧 generation。准备在持久化前失败时，本轮目录要等后续成功准备回收 | `Client::prepare_job`、`gc_staging_generations` | 保障 | 高 | 宽泛删除 staging 会破坏相邻任务；从不重试则失败 generation 可能占空间 | 准备失败后重试、非法 job ID、符号链接、相邻 job 不受影响 |
