...
| MED-P-009 | Client 固定 Foundation Client `0.7.1` 与完整 Git revision；protocol 也固定 Server 仓库完整 revision，无相邻工作区来源 | `sarmg-client.toml`、crate manifests、Cargo.lock | 保障 | 高 | 平台或 wire 行为可能随未固定依赖漂移 | 独立 checkout 的 locked 构建、合同与 ABI 测试 |
