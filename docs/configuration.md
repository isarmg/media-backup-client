...
5. 打开“设置”，选择“自动备份”“仅 Wi-Fi”“后台仅充电时上传”；
...
7. 勾选至少一个需要自动备份的相册，再点击“保存备份偏好”；未保存的选择不参与自动扫描；
8. 将浏览缓存设置为 `64`～`1024 MiB`。

iOS 通过 PhotoKit 相册选择自动备份范围，没有 Android 的“仅相机目录”开关。备份偏好也可以在配对前保存；浏览云端图库不会代替“保存备份偏好”。后台执行时间由系统决定；低电量、关闭后台刷新或照片权限变化都可能推迟上传。
